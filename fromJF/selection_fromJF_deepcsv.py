...
gInterpreter.Declare("""
      ...
      using Vfloat = const ROOT::RVec<float>&;
      using Vint = const ROOT::RVec<int>&;
      auto firstjet( UInt_t njetgood,Vint good, Vint jetbotind, Vfloat jet_pt, Vfloat charm_tag1, Vfloat charm_tag2) {
            int ind = -1;
            float pT = -1.;
            float tagC = -1.;
            for (unsigned int i=0; i<njetgood; ++i){
              tagC = charm_tag1[good[i]] + charm_tag2[good[i]];
              if (good[i]!=good[jetbotind[0]] && good[i]!=good[jetbotind[1]] && tagC > pT) {
                    ind = good[i];
                    pT = tagC;
              }
            }
      ...
      auto firstjet_hig( UInt_t njetgood,Vint good, Vint jetbotind, Vfloat jet_pt, Vfloat charm_tag) {
            int ind = -1;
            float pT = -1.;
            for (unsigned int i=0; i<njetgood; ++i){
              if (good[i]!=good[jetbotind[0]] && good[i]!=good[jetbotind[1]] && charm_tag[good[i]] > pT) {
                    ind = good[i];
                    pT = charm_tag[good[i]];
              }
            }
...
gInterpreter.Declare("""
      ...
      using Vfloat = const ROOT::RVec<float>&;
      using Vint = const ROOT::RVec<int>&;
      auto secondjet( UInt_t njetgood,Vint good, Vint jetbotind, Vfloat jet_pt, Vfloat qgl, int ind_one) {
            int ind = -1;
            float pT = -10.;
            for (unsigned int i=0; i<njetgood; ++i){
              if (jet_pt[good[i]] > pT && good[i]!=ind_one && good[i]!=good[jetbotind[0]] && good[i]!=good[jetbotind[1]] && qgl[good[i]] > 0.25) {
                    ind = good[i];
                    pT = jet_pt[good[i]];
              }
            }
...
for s in samples:
        ...
        if s[0:3] == "hig":
                df[s] = df[s].Define('JetOneInd','firstjet_hig(nJetGood, JetGoodInd, JetBotInd, Jet_pt_nom, Jet_btagDeepFlavC)')
        else:
                df[s] = df[s].Define('JetOneInd','firstjet(nJetGood, JetGoodInd, JetBotInd, Jet_pt_nom, Jet_btagDeepFlavCvB, Jet_btagDeepFlavCvL)')
        df[s] = df[s].Define('JetTwoInd','secondjet(nJetGood, JetGoodInd, JetBotInd, Jet_pt_nom, Jet_qgl, JetOneInd)')
        df[s] = df[s].Filter('JetTwoInd > -1')
        ### hists definitions
        df[s] = df[s].Define('jet_muon_pt','Jet_pt_nom[JetOneInd]')
        df[s] = df[s].Define('jet_muon_nmu','Jet_nMuons[JetOneInd]')
        df[s] = df[s].Define('jet_muon_mass','Jet_mass_nom[JetTwoInd]')
        df[s] = df[s].Define('jet_notmuon_pt','nJetGood>1 ? Jet_pt_nom[JetTwoInd] : 0')
        df[s] = df[s].Define('jet_muon_eta','Jet_eta[JetOneInd]')
        df[s] = df[s].Define('jet_notmuon_eta','nJetGood>1 ? Jet_eta[JetTwoInd] : 0')
        df[s] = df[s].Define('jet_notmuon_mass','nJetGood>1 ? Jet_mass_nom[JetTwoInd] : 0')
        df[s] = df[s].Define('jet_notmuon_qgl','nJetGood>1 ? Jet_qgl[JetTwoInd] : 0')
        df[s] = df[s].Define('jet_notmuon_nmu','nJetGood>1 ? Jet_nMuons[JetTwoInd] : 0')
        df[s] = df[s].Define('InvM_2jets','nJetGood>1 ? InvariantM(Jet_pt_nom[JetOneInd],Jet_eta[JetOneInd],Jet_phi[JetOneInd],0.,Jet_pt_nom[JetTwoInd],Jet_eta[JetTwoInd],Jet_phi[JetTwoInd],0.) : 0')
...
if mode == "mc":
	for s in samples:
		df[s] = df[s].Define('jet_muon_flavourH','Jet_hadronFlavour[JetOneInd]')
		df[s] = df[s].Define('jet_notmuon_flavourH','Jet_hadronFlavour[JetTwoInd]')
		df[s] = df[s].Define('jet_muon_flavourP','Jet_partonFlavour[JetOneInd]')
		df[s] = df[s].Define('jet_notmuon_flavourP','Jet_partonFlavour[JetTwoInd]')
...
for s in samples:
        if args.type == "mc":
               ...
               df_E[s] = df_E[s].Define('weightSSOS_final','weight_aux*lep_id_sf*btag_sf*puWeight*PUjetID_SF*lep_trig_sf')
        else:
...
for s in samples:
                path_hist = '/nfs/cms/vazqueze/hists_ttbar/hists/higgs/fromJF/histstt_deeptag_fromJF_'+s+'.root'
