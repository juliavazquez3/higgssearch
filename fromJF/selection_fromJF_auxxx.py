  ...
  if args.type == "mc":
    for syst in list_jetsyst:  
        ...
        if p == "M":
          ...
          for f in [f1 for f1 in listdir(folder1) if (isdir(join(folder1, f1)) and f1[0:9] == "data_mu_a")]:
...
for s in samples:
    for syst in list_jetsyst:
        ...
        df[syst][s] = df[syst][s].Filter('nJetGood == 4')
        df[syst][s] = df[syst][s].Filter('nMuon == 2')
...
for s in samples:
   if args.wcs:
       ...
       if channel=="sl_os":
           path = '/pnfs/ciemat.es/data/cms/store/user/juvazque/data_further_analysis/wcs_btagMM_chi/slos/'
   else:
       if (channel[0:2]=="sl"):
