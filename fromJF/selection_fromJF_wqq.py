...
for s in samples:
        df[s] = df[s].Define('JetQInd','JetInds(nJet, JetGoodInd, Jet_pt, Jet_eta, Jet_phi, JetBotInd, Jet_qgl)')
...
if mode == "mc":
	for s in samples:
		df[s] = df[s].Define('jet_muon_flavourH','Jet_hadronFlavour[JetQInd[0]]')
		df[s] = df[s].Define('jet_notmuon_flavourH','Jet_hadronFlavour[JetnotMuonInd]')
		df[s] = df[s].Define('jet_muon_flavourP','Jet_partonFlavour[JetQInd[0]]')
...
cond1 = 'fabs(jet_muon_flavourP) == 4 || fabs(jet_notmuon_flavourP) == 4'
cond2 = 'fabs(jet_muon_flavourP) == 5 || fabs(jet_notmuon_flavourP) == 5'
cond3 = 'fabs(jet_muon_flavourP) == 3 || fabs(jet_muon_flavourP) == 2 || fabs(jet_muon_flavourP) == 1'
cond4 =	'fabs(jet_notmuon_flavourP) == 3 || fabs(jet_notmuon_flavourP) == 2 || fabs(jet_notmuon_flavourP) == 1'

if mode == "mc":
        for s in samples:
                if (s[0:8] == "ttbar_sl" and (s[-1]=="6" or s[-1]=="7" or s[-1]=="8" or s[-1]=="B")):
                        df[s+"_charm"] = df[s].Filter(cond1)
                        df[s+"_bottom"] = df[s].Filter('('+cond2+') && !('+cond1+')')
...
for s in samples:
        ...
        hist_jet_muon_pt_M[s] = df_M[s].Histo1D(("jet_1_pt_M","",50,20,120),"jet_1_pt","weightSSOS_final")
        ...
        hist_jet_muon_pt_E[s] = df_E[s].Histo1D(("jet_1_pt_E","",50,20,120),"jet_1_pt","weightSSOS_final")
...
for s in samples:
                path_hist = '/nfs/cms/vazqueze/hists_ttbar/hists/higgs/fromJF/wqq/histstt_wqq_fromJF_'+s+'.root'
