...
if args.type == "mc":
   term3["nom"] = "/pnfs/ciemat.es/data/cms/store/user/juvazque/PreprocessRDF_last_version/"
else:
...
for s in samples:
  for syst in list_jetsyst:
    ...
    df[syst][s] = ROOT.RDataFrame("Events",set(archives["nom"][s]))
...
gInterpreter.Declare("""
      ...
      using Vfloat = const ROOT::RVec<float>&;
      using Vint = const ROOT::RVec<int>&;
      auto jetptrecut(Vint good, Vint bgood, Vfloat pt, Vfloat eta, Vfloat phi, Vfloat qgl) {
            bool cond1 = true;
            int aux1 = -1;
            int aux2 = -1;
            vector<float> vb;
            bool condb = pt[good[bgood[0]]]>25. && pt[good[bgood[1]]]>25.;
            auto vec = JetInds(4,good,pt,eta,phi,bgood,qgl);
            bool cond2 = pt[vec[0]]>25. && pt[vec[1]]>25.;
            cond1 = condb && cond2;
            return cond1;
      };
""")
...
##### Cut on jet pT
for s in samples:
      df[syst][s] = df[syst][s].Filter('jetptrecut(JetGoodInd, JetBotInd, Jet_pt_nom, Jet_eta, Jet_phi, Jet_qgl)')
...
if args.type == "mc":
    for s in samples:
        for syst in list_jetsyst:
               ...
               df[syst][s] = df[syst][s].Define('lep_iso_sf','nMuonGood>0 ? musf_tight_iso[MuonGoodInd[0]] : 1.')
               df[syst][s] = df[syst][s].Define('lep_iso_sf_up','nMuonGood>0 ? musf_tight_iso_up[MuonGoodInd[0]] : 1.')
               df[syst][s] = df[syst][s].Define('lep_iso_sf_down','nMuonGood>0 ? musf_tight_iso_down[MuonGoodInd[0]] : 1.')
...
if channel[0:2] == "sl": observable_names = observable_names + ["muon_jet_pt","muon_jet_eta","deltaR_jet1_muon","deltaR_muon_tau","muon_jet_z","muon_jet_iso",
          ...
          "jet_muon_nmu","jet_notmuon_nmu","muon_jet_xy","muon_jet_dz"]
...
#########   observable_names = ["second_muon_pt","second_el_pt"]
...
if mode == "mc":
   brlist = brlist + ["nGenJet", "GenJet_eta", "GenJet_hadronFlavour", "GenJet_mass", "GenJet_partonFlavour", "GenJet_phi", "GenJet_pt",
       ...
       "var_xsec","var_lumi","lumi_data","weight_lumi","ttsl_lepflav","ttdl_lepflav","Jet_pt_up","Jet_pt_down"]
   if (channel[0:2]=="sl"): brlist = brlist + ["muon_jet_genindx"]

#for s in samples:
#   if args.wcs:
#       if channel[0:2]=="sl":
#           path = '/pnfs/ciemat.es/data/cms/store/user/juvazque/data_further_analysis/wcs_btagMM_chi/sl/'
#       elif channel=="antisl":
#           path = '/pnfs/ciemat.es/data/cms/store/user/juvazque/data_further_analysis/wcs_btagMM_chi/antisl/'
#       else:
#           path = '/pnfs/ciemat.es/data/cms/store/user/juvazque/data_further_analysis/wcs_btagMM_chi/'
...
if chan == "nobtag": term1 = "nobtag/"
  ...
  if args.wcs:
     term1 = "btagMM/chi_test/wcs_last/nochitest/"
  else:
     term1 = "btagMM/chi_test/nochitest/"
elif chan == "lepton50": term1 = "lepton50/"
elif chan == "btagMM_chitest": 
  if args.wcs:
     term1 = "btagMM/chi_test/wcs_last/"
  else:
