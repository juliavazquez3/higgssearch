...
for s in samples:
	...
	df[s] = df[s].Define('jet_muon_btag','Jet_btagDeepB[JetMuonInd[0]]')
	df[s] = df[s].Define('jet_notmuon_btag','Jet_btagDeepB[JetnotMuonInd]')
	...
	df[s] = df[s].Define('InvM3_aux','InvMassBot(JetBotInd, JetMuonInd, JetGoodInd, Jet_pt, Jet_eta, Jet_phi, JetnotMuonInd, Jet_mass)')
...
for s in samples:
	...
	if args.type == "mc":
		...
		if args.type == "mc":
			df[s] = df[s].Filter('nMuonGood>0 ? '+ muon_trig[years[0]] +' : '+ el_trig[years[0]])
		else:
			print('no data trigger')
...
for s in samples:
        ...
        hist_events_M_os[s] = df_M_os[s].Histo1D(("event_M_os","",1000000,0,1000000),"event")
        hist_events_E_os[s] = df_E_os[s].Histo1D(("event_E_os","",1000000,0,1000000),"event")
...
for s in samples:
                if args.ssos: path_hist = '/nfs/cms/vazqueze/hists_ttbar/hists/higgs/ssos/histstt_SL_v1v2vBWPSSOS_'+s+'.root'
                else: path_hist = '/nfs/cms/vazqueze/hists_ttbar/hists/higgs/histstt_SL_v1v2vBWP_'+s+'.root'
                ...
                hist_events_M_os[s].Write()
                hist_events_E_os[s].Write()
